        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {     try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT last_updated_date,date_activated,pan,seq_nr,card_status,card_program,issuer_nr,branch_code,date_issued,customer_id,hold_rsp_code,last_updated_user FROM pc_cards_1_A  where pan = '" + card + "' ", obj.conn);
            ...
            String row = (Convert.ToDouble(str4.Text) + 1).ToString();
                ...
                try
                {
                    using (SqlConnection sqlCon = new SqlConnection(obj.locate1))
                    {
                        ...
                        SqlCommand sqlCmd = new SqlCommand(query, sqlCon);
                        sqlCmd.ExecuteNonQuery();
                        string query2 = " insert into postilion_hold_data (action,id,branch_code,hold_rsp_code,customer_id ,place_hold ,account_id ,pan,card_status ,reason_for_reason ,mail_destination ,seq_nr ,expiry_date ,title,first_name,middle_initial,last_name,name_on_card,other,account_product,mobile,issuer_nr,account_type,last_updated_date,last_updated_user,address_1_1,city) values ('EDIT CARD','" + row + "','" + TextBox10.Text + "','','" + customer_id_text.Text + "','" + DropDownList1.Text + "','','' , '','','','','','','','','' , '','','','','','','','','' , '')";
